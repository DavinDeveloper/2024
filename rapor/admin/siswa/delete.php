		...
		$data_siswa = mysqli_fetch_assoc($check_siswa);
				if (isset($_POST['delete'])) {
					if (mysqli_num_rows($check_siswa) == 0) {
						$msg_type = "error";
						$msg_content = "<b>Gagal:</b> Siswa tidak tersedia atau sudah terhapus.";
					} else {
					    $delete = mysqli_query($db, "DELETE FROM pts WHERE nis = '$post_id'");
					    $delete = mysqli_query($db, "DELETE FROM pas WHERE nis = '$post_id'");
						$delete = mysqli_query($db, "DELETE FROM users WHERE id = '$post_id'");
				        if ($delete == TRUE) {
							$msg_type = "success";
							$msg_content = "<b>Berhasil:</b> Siswa ".$data_siswa['nama']." berhasil dihapus.<br />";
						} else {
							$msg_type = "error";
                                    ...
                                    <div class="card-header text-uppercase">Hapus Data Siswa</div>
										...
										<form class="form-horizontal" role="form" method="POST">
											<div class="form-group">
												...
												<div class="col-md-12">
													<input type="text" name="nama" class="form-control" placeholder="Masukkan Nama Siswa" value="<? echo $data_siswa['nama']; ?>" readonly>
												</div>
											</div>
											<div class="form-group">
												<label class="col-md-2 control-label">KELAS *</label>
												<div class="col-md-12">
													<input type="text" name="kelas" class="form-control" placeholder="Masukkan Kelas Siswa" value="<? echo $data_siswa['kelas']; ?>" readonly>
												</div>
											</div>
											<div class="form-group">
