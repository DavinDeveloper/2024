		...
		header("Location: ".$cfg_baseurl);
				...
				if (isset($_POST['tambah'])) {
					...
					if (mysqli_num_rows($check_guru) > 0) {
						...
						$insert_guru = mysqli_query($db, "INSERT INTO users (username, nomor, password, nisn, status, nama, kelas, pelajaran, jurusan) VALUES ('$post_username', '$post_nip', '".md5($post_nip)."', '$post_nip', 'Guru', '$post_nama', '$post_kelas', '$post_pelajaran', '$post_jurusan')");
				        if ($insert_guru == TRUE) {
							$msg_type = "success";
										...
										<form class="form-horizontal" role="form" method="POST">
											...
											<div class="form-group">
                                              <label class="col-md-12 control-label">MATA PELAJARAN</label>
                                    			...
                                    			<option value="">Pilih</option>
                                                <?php
                                                $check_pelajaran = mysqli_query($db, "SELECT * FROM pelajaran WHERE jurusan IS NULL ORDER BY nama ASC");
