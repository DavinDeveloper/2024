	...
	if (isset($_POST['login'])) {
		...
		if (empty($post_username) || empty($post_password)) {
			...
			if (empty($post_password)) {
    			$msg_type = "error";
    			$msg_content = "<strong>Gagal : </strong>Please fill all input.";
    		} else if (mysqli_num_rows($check_user) == 0) {
				$msg_type = "error";
				$msg_content = "<strong>Gagal : </strong>Username or email not found/invalid.";
				$data_user = mysqli_fetch_assoc($check_user);
			} else if ($data_user['status'] == 'Suspended') {
				$msg_type = "error";
				$msg_content = "<strong>Gagal : </strong>Your account has been suspended.";
			} else {
				$data_user = mysqli_fetch_assoc($check_user);
				if (md5($post_password) <> $data_user['password']) {
					$msg_type = "error";
					$msg_content = "<strong>Gagal : </strong>Wrong username or password.";
				} else {
